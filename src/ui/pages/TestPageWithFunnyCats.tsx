...
type Resp = {
  asdf: {data: {id: string, attributes: {
...
export const TestPageWithFunnyCats: React.FC<{}> = () => {
  const { id } = useParams();
  const { loading, error, data } = useQuery<Resp>(query, { variables: { id: id } });
  ...
  return (
    <ContentSpecial>
        ...
        <div dangerouslySetInnerHTML={createMarkup(data!.asdf.data.attributes.toolbarVersion)} />
        {/* <ReactMarkdown>
